...
static K_INT32 numspeakers, bytespersample;
static float recipsamp;
static celltype cell[MAXCELLS];
static K_INT16 wavtable[WAVPREC*3];
...
K_INT32 lplc[9] = {0,0,0,0,0,0,0,0,0};   //Samples to delay on left speaker
K_INT32 rplc[9] = {0,0,0,0,0,0,0,0,0};   //Samples to delay on right speaker

K_INT32 nlvol[9], nrvol[9];
K_INT32 nlplc[9], nrplc[9];
K_INT32 rend = 0;
#define FIFOSIZ 256
...
#ifdef USING_ASM
static _inline void ftol (float f, K_INT32 *a)
{
    _asm
	{
	    mov eax, a
...
#else
static void ftol(float f, K_INT32 *a) {
    *a=f;
}
#endif
...
static K_INT32 waveform[8] = {WAVPREC,WAVPREC>>1,WAVPREC,(WAVPREC*3)>>2,0,0,(WAVPREC*5)>>2,WAVPREC<<1};
static K_INT32 wavemask[8] = {WAVPREC-1,WAVPREC-1,(WAVPREC>>1)-1,(WAVPREC>>1)-1,WAVPREC-1,((WAVPREC*3)>>2)-1,WAVPREC>>1,WAVPREC-1};
static K_INT32 wavestart[8] = {0,WAVPREC>>1,0,WAVPREC>>2,0,0,0,WAVPREC>>3};
...
void cellon (K_INT32 i, K_INT32 j, celltype *c, unsigned char iscarrier)
{
    K_INT32 frn, oct, toff;
    float f;

    frn = ((((K_INT32)adlibreg[i+0xb0])&3)<<8) + (K_INT32)adlibreg[i+0xa0];
    oct = ((((K_INT32)adlibreg[i+0xb0])>>2)&7);
...
void cellfreq (K_INT32 i, K_INT32 j, celltype *c)
{
    K_INT32 frn, oct;
...
static K_INT32 initfirstime = 0;
void adlibinit (K_INT32 dasamplerate, K_INT32 danumspeakers, K_INT32 dabytespersample)
{
    K_INT32 i, j, frn, oct;
	...
	{
	    wavtable[i] =
		wavtable[(i<<1)  +WAVPREC] = (K_INT16)(16384*sin((float)((i<<1)  )*PI*2/WAVPREC));
	    wavtable[(i<<1)+1+WAVPREC] = (K_INT16)(16384*sin((float)((i<<1)+1)*PI*2/WAVPREC));
	}
	...
	for(j=6;j>=0;j--)
	    for(i=0;i<16;i++)
	    {
		oct = (K_INT32)ksl[j+1][i]-8; if (oct < 0) oct = 0;
...
void adlib0 (K_INT32 i, K_INT32 v)
{
...
#ifdef USING_ASM
static K_INT32 fpuasm;
static float fakeadd = 8388608.0+128.0;
static _inline void clipit8 (float f, K_INT32 a)
{
    _asm
	{
	    mov edi, a
...
static _inline void clipit16 (float f, K_INT32 a)
{
    _asm
	{
	    mov eax, a
...
static void clipit16(float f,K_INT16 *a) {
...
void adlibgetsample (unsigned char *sndptr, K_INT32 numbytes)
{
    K_INT32 i, j, k=0, ns, endsamples, rptrs, numsamples;
    celltype *cptr;
    float f;
    K_INT16 *sndptr2=(K_INT16 *)sndptr;
