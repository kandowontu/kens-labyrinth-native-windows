...
/* The DOS-era renderer updates modal screens by drawing directly to the
   visible buffer and calling glFlush().  Front-buffer writes are not reliably
   presented by composited Windows desktops (and by SDL compatibility layers),
   so mirror those draws into both buffers and present the back buffer on
   flush.  Both buffers then remain identical across the swap, preserving the
   incremental menu/high-score/level-end drawing semantics. */
static int frontbufferdrawing = 0;
...
void openGLDrawFront(void) {
    frontbufferdrawing=1;
    glDrawBuffer(GL_FRONT_AND_BACK);
}

void openGLDrawBack(void) {
    frontbufferdrawing=0;
