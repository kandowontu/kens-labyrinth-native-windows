...
static K_UINT16 clamp_noclip_position(K_INT32 position)
{
    /* Keep the player in the inner 62x62 cells because several original
       action checks inspect the four neighbouring board cells directly. */
    if (position < 1024)
	position = 1024;
    if (position > 64511)
	position = 64511;
...
int main(int argc,char **argv)
{
    ...
    K_INT32 templong, cheatdx, cheatdy;
...
#ifdef WIN32
    /* All original data access is relative.  Pin it to the installed EXE so
       launching from Explorer, a shortcut, or another working directory is
       deterministic. */
	...
	if (cheat_menu_requested)
	{
	    cheat_menu_requested = 0;
	    newkeystatus[newkeydefs[CONTROL_CHEAT_MENU]] = 0;
	    /* Present a complete game frame before entering the modal overlay.
	       This mirrors the proven Escape-menu path and avoids front-buffer
	       writes that Windows' desktop compositor may never display. */
	...
	{
	    if (newkeystatus[newkeydefs[14]] > 0)
	    {
		if (lastbarchange == 0)
		{
		    if (vidmode == 0)
...
/*		for(i=lside;i<rside;i++)
	...
	clockspd=clockspeed;
	if (debugmode)
	  fprintf(stderr,"TIMING: Clock ticks: %d.\n", (int)clockspd);
	...
	if (death == 4095)
	{
	    ...
	    if ((newkeystatus[newkeydefs[9]]|newkeystatus[newkeydefs[10]]|newkeystatus[newkeydefs[11]]) > 0)
	    {
		j = bulchoose;
		if (newkeystatus[newkeydefs[9]] > 0) bulchoose = 0;
		if (newkeystatus[newkeydefs[10]] > 0) bulchoose = 1;
		if (newkeystatus[newkeydefs[11]] > 0) bulchoose = 2;
	    ...
	    if (((newkeystatus[newkeydefs[8]] > 0) || ((bstatus&1) > 0)))
	    {
	...
	for(i=0;i<bulnum;i++)
	{
		...
		if (bulstat[i] > 0)
		{
		    bulx[i] += x;
		    buly[i] += y;
			if ((bultype[bulkind[i]] == 2) && (death == 4095) &&
			    (m < 4) && !cheat_godmode)
		    {
	...
	for(i=0;i<mnum;i++)
	{
		...
		if ((templong < 768) && (death == 4095) &&
		    (mstat[i] != mondog) && !cheat_godmode)
		{
	...
	if (death == 4095)
	{
	    ...
	    if ((newkeystatus[newkeydefs[4]] == 0))
	    {
		j = clockspd;
		if (j > 16)
		    j = 16;
		if (newkeystatus[newkeydefs[2]] > 0)
		{
		    angvel -= j;
		    if (angvel < -36)
			angvel = -36;
		...
		if (newkeystatus[newkeydefs[3]] > 0)
		{
		    angvel += j;
		    if (angvel > 36)
			angvel = 36;
	...
	if (compass > 0)
	    showcompass(ang);
	if ((newkeystatus[newkeydefs[5]] > 0) && (death == 4095))
	{
	    hvel -= (clockspd>>1);
	    if (hvel < -3)
		hvel = -3;
	...
	if ((newkeystatus[newkeydefs[6]] > 0) && (death == 4095))
	{
	    hvel += (clockspd>>1);
	    if (hvel > 3)
		hvel = 3;
	...
	waterstat = 0;
	if (((newkeystatus[newkeydefs[12]] > 0) || 
	...
	maxvel = 128;
	if (newkeystatus[newkeydefs[7]] > 0)
	    maxvel = 256;
	...
	if (((vel+myvel) != 0) || (svel != 0))
	{
	    ...
	    if ((vel+myvel) != 0)
	    {
		cheatdx = ((K_INT32)(vel+myvel)*clockspd*sintable[(ang+512)&2047])>>19;
		cheatdy = ((K_INT32)(vel+myvel)*clockspd*sintable[ang])>>19;
		if (cheat_noclip)
		{
		    posx = clamp_noclip_position((K_INT32)posx+cheatdx);
		    posy = clamp_noclip_position((K_INT32)posy+cheatdy);
		}
		else
		{
		    posx += (K_INT16)cheatdx;
		    posy += (K_INT16)cheatdy;
		    checkhitwall(oldposx,oldposy,posx,posy);
		}
	    }
	    if (svel != 0)
	    {
		cheatdx = ((K_INT32)svel*clockspd*sintable[ang])>>19;
		cheatdy = ((K_INT32)svel*clockspd*sintable[(ang+1536)&2047])>>19;
		if (cheat_noclip)
		{
		    posx = clamp_noclip_position((K_INT32)posx+cheatdx);
		    posy = clamp_noclip_position((K_INT32)posy+cheatdy);
	...
	if (paletterenderer) {
	    softwareUpdateOverlayPalette(255, 1, spritepalette+765);
	}
	...
	if (life < 4095)
	    if (((i == meal) || (i == honey) || (i == fries) || (i == firstaid)) || (newkeystatus[newkeydefs[13]] > 0) || ((keystatus[31] > 0) && (cheatkeysdown == 1)))
	    {
		if ((keystatus[31] > 0) && (cheatkeysdown == 1))
		    life += 320, cheated++;
		if (newkeystatus[newkeydefs[13]] > 0)
		{
		    life += 320;
		    scorecount >>= 1;
		    drawscore(scorecount);
	...
	if ((i == stairs) || (won == 1) || cheat_next_board ||
	    (cheat_warp_board >= 0) ||
	    ((keystatus[48] > 0) && (cheatkeysdown == 1)))
	{
	    cheatwarp = cheat_warp_board;
	    if (cheatwarp >= 0)
	    {
		if (cheatwarp >= numboards)
		    cheatwarp = numboards-1;
		cheat_warp_board = -1;
		cheated++;
	    }
	    if (cheat_next_board)
	    ...
	    won = 0;
	    musicoff();
	    if ((boardnum < numboards-1) || (cheatwarp >= 0))
	    {
		if ((i == stairs) && (cheatwarp < 0))
		{
		    if (cheated == 0)
			ksay(11);
		    else
	...
	if (death == 4095)
	{
	    if ((i == fan) && (capetime[0] < totalclock) &&
		(capetime[1] < totalclock) && !cheat_godmode)
	    {
		life -= (clockspd<<3);
		if (life <= 0)
		{
	    ...
	    if (((i&1023) == hole) && (incenter == 1) && !cheat_godmode)
	    {
		life = 0;
		drawlife();
		death = 4094;
	...
	if (newkeystatus[newkeydefs[15]] > 0)
	{
	...
	if (newkeystatus[newkeydefs[CONTROL_SCREENSHOT]] > 0) {
	    gfxScreencapture();
	    newkeystatus[newkeydefs[CONTROL_SCREENSHOT]]=0;
	}
	if (ototclock == 0)
	{
	...
	if (newkeystatus[newkeydefs[16]] > 0)
	{
	    ...
	    SDL_UnlockMutex(soundmutex);
	    newkeystatus[newkeydefs[16]] = 0;
	}

	/* [ESCAPE] to main menu... */

	if (newkeystatus[newkeydefs[17]] == 1)
	{
	    if (ototclock > 1)
	    {
		#ifdef WIN32
	...
	if (newkeystatus[newkeydefs[CONTROL_SOUND_DOWN]]) {
	    soundvolume-=(clockspd>>1);
	    if (soundvolume<0) soundvolume=0;
	    soundvolumevisible=240;
	}
	if (newkeystatus[newkeydefs[CONTROL_SOUND_UP]]) {
	    soundvolume+=(clockspd>>1);
	    if (soundvolume>256) soundvolume=256;
	    soundvolumevisible=240;
	}
	if (newkeystatus[newkeydefs[CONTROL_MUSIC_DOWN]]) {
	...
	if (newkeystatus[newkeydefs[CONTROL_MUSIC_UP]]) {
	    musicvolume+=(clockspd>>1);
	...
	if (newkeystatus[newkeydefs[CONTROL_GAMMA_UP]]) {
	...
	if (newkeystatus[newkeydefs[CONTROL_GAMMA_DOWN]]) {
	  gammalevel*=pow(1.01,-clockspd);
	...
	/* I have not the foggiest idea why drawing the clock every frame
	   eliminates judder when using vsync on my Radeon, but my guess is
	   that it prevents Catalyst from doing something clever. If I leave
	   the if here, I get one dropped frame every time the time is drawn.
	*/
	drawtime(scoreclock);
