...
void oldmain(void)
{
    ...
    while (newkeystatus[newkeydefs[17]] == 0)
    {
	PollInputs();
	/* Check cheat keys... */
...
//			    statusbaralldraw();
	...
	if (death == 4095)
	{
	    ...
	    {
		if ((newkeystatus[newkeydefs[2]]) && (angvel > -40))
		    angvel -= 14;
		if ((newkeystatus[newkeydefs[3]]) && (angvel < 40))
		    angvel += 14;
	    }
	    else
	    {
		if (newkeystatus[newkeydefs[2]])
		{
		    svel += 40;
		    if (svel > maxvel)
			svel = maxvel;
	...
	if ((rogermode > 0) && (death == 4095) && (newkeystatus[newkeydefs[2]] == 0) && (newkeystatus[newkeydefs[3]] == 0))
	    angvel = 0;
	ang = (ang+2048+(((angvel+mxvel)*clockspd)>>3))&2047;
	if (compass > 0)
	    oldshowcompass(ang);
	if ((newkeystatus[newkeydefs[5]] > 0) && (hvel > -5) && (death == 4095))
	    hvel-=2;
	if ((newkeystatus[newkeydefs[6]] > 0) && (hvel < 5) && (death == 4095))
	    hvel+=2;
	if (hvel < 0)
	    hvel++;
	if (hvel > 0)
	...
	if (((newkeystatus[newkeydefs[12]] > 0) || ((bstatus&2) > 0)) && (death == 4095))
	{
	    x = (posx>>10);
	    y = (posy>>10);
	    if (!inhibitrepeat) {
	...
	if (((vel+myvel) != 0) || (svel != 0))
	{
	    ...
	    if (svel != 0)
	    {
		posx += (int)(((long)svel*clockspd*sintable[ang])>>19);
		posy += (int)(((long)svel*clockspd*sintable[(ang+1536)&2047])>>19);
	...
	if (!cheatkeysdown)
	{
	    if ((newkeystatus[newkeydefs[13]] > 0) && (death == 4095))
	    {
		j = pageoffset;
	...
	/*	if ((scoreclock%240) < clockspd)
	 Workaround for judder; see equivalent code in lab3d.c. */
	olddrawtime(scoreclock);
	gfxSwapBuffers( );
    }
    musicoff();
...
void olddrawlife(void)
{
	...
	gfxUploadPartialOverlay(x,y,blocks<<2,2);
    }
...
K_INT16 oldloadgame(K_INT16 gamenum)
{
    ...
    readLE32(fil,&nownote_long,4);
    nownote=nownote_long;
...
void oldstatusbaralldraw(void)
{
    ...
    gfxUploadPartialOverlay(0,(statusbaryoffset-1),361,1);
    for(i=0;i<361;i++)
	screenbuffer[i+(statusbaryoffset+32)*screenbufferwidth]=0x50;
    gfxUploadPartialOverlay(0,(statusbaryoffset+32),361,1);
    if (vidmode == 1)
    {
