...
void preparesound(void *dasnd, long numbytestoprocess)
{
	...
	dasnd = (void *)(((unsigned char *)dasnd)+i);
	prepcnt -= i;
...
void statusbaralldraw()
{
    ...
    /* The legacy VGA split used a solid separator scanline here. At modern
       scaling factors that becomes a conspicuous band below the HUD. Let the
       3-D view show through this overlay row instead. */
    for(i=0;i<361;i++)
	screenbuffer[i+240*screenbufferwidth]=0xff;
...
/* Keep the native port's persistent cheat toggles supplied.  The original
   game has one-shot key chords for most of these; this layer makes them easy
   to use without changing the underlying inventory and power-up systems. */
...
void applycheats()
{
    ...
    if ((cheat_godmode || cheat_noclip || cheat_infinite_money ||
	 cheat_all_keys || cheat_max_weapons || cheat_infinite_powerups ||
	 cheat_extra_lives || cheat_compass) && (cheated == 0))
	cheated = 1;
    ...
    if (cheat_infinite_money && (coins != 999))
    {
	coins = 999;
	redraw = 1;
    }
    if (cheat_all_keys && ((keys[0] < 9) || (keys[1] < 9)))
    {
	keys[0] = 9;
	keys[1] = 9;
	redraw = 1;
    }
    if (cheat_max_weapons &&
	((firepowers[0] != 6) || (firepowers[1] != 6) ||
	 (firepowers[2] != 6) || (lightnings != 6)))
    {
	firepowers[0] = 6;
	firepowers[1] = 6;
	firepowers[2] = 6;
    ...
    if (cheat_extra_lives && (lifevests != 9))
    {
	lifevests = 9;
	redraw = 1;
    }
    if (cheat_compass && (compass == 0))
    {
	compass = 1;
	redraw = 1;
    }
    if (cheat_infinite_powerups)
    {
	if (purpletime < totalclock+4800)
	{
	    purpletime = totalclock+9600;
	    redraw = 1;
	}
	if (greentime < totalclock+4800)
	{
	    greentime = totalclock+9600;
	    redraw = 1;
	}
	if (capetime[0] < totalclock+4800)
	{
	    capetime[0] = totalclock+9600;
	    redraw = 1;
	}
	if (capetime[1] < totalclock+4800)
	{
	    capetime[1] = totalclock+9600;
	    redraw = 1;
	}
    }
...
static K_INT16 allcheatsenabled()
{
    return cheat_godmode && cheat_noclip && cheat_infinite_money &&
	cheat_all_keys && cheat_max_weapons && cheat_infinite_powerups &&
	cheat_extra_lives && cheat_compass;
}

static K_INT16 anycheatsenabled()
{
    return cheat_godmode || cheat_noclip || cheat_infinite_money ||
	cheat_all_keys || cheat_max_weapons || cheat_infinite_powerups ||
	cheat_extra_lives || cheat_compass;
...
static void setallcheats(K_INT16 enabled)
{
    if (!enabled && cheat_compass)
	compass = 0;
    cheat_godmode = enabled;
    cheat_noclip = enabled;
    cheat_infinite_money = enabled;
    cheat_all_keys = enabled;
    cheat_max_weapons = enabled;
    cheat_infinite_powerups = enabled;
    cheat_extra_lives = enabled;
    cheat_compass = enabled;
}

static void drawcheattoggle(const char *label, K_INT16 enabled, K_INT16 y)
{
    sprintf((char *)&textbuf[0],"%-25s %s",label,enabled ? "ON" : "OFF");
    textprint(67,y,32);
}

#define CHEAT_MENU_ITEMS 15
#define CHEAT_MENU_FIRST_Y 29
#define CHEAT_MENU_ROW_HEIGHT 10

static K_INT16 cheatchaptercount()
{
    K_INT16 chapters = (numboards+9)/10;
    if (chapters < 1)
	chapters = 1;
    return chapters;
}

static K_INT16 cheatlevelsforchapter(K_INT16 chapter)
{
    K_INT16 levels = numboards-(chapter*10);
    if (levels > 10)
	levels = 10;
    if (levels < 1)
	levels = 1;
...
static const char *cheatchaptername(K_INT16 chapter)
{
...
static void drawcheatselector(K_INT16 selection, K_INT16 n, K_INT16 frame)
{
    if (lab3dversion)
	statusbardraw(16+(frame/2)*16,0,15,15,67-n,
		selection*CHEAT_MENU_ROW_HEIGHT+CHEAT_MENU_FIRST_Y-2+n,85);
    else if (frame < 3)
	statusbardraw(20+frame*14,32,13,13,48-n,
		selection*CHEAT_MENU_ROW_HEIGHT+CHEAT_MENU_FIRST_Y-1+n,
		statusbarback);
    else
	statusbardraw(20+(frame-3)*14,46,13,13,48-n,
		selection*CHEAT_MENU_ROW_HEIGHT+CHEAT_MENU_FIRST_Y-1+n,
		statusbarback);
}

/* Unlike the original menus, this selector redraws the whole modal frame to
   the back buffer and swaps it into view.  Direct GL_FRONT drawing is not
   reliably presented by the Windows compositor, which can otherwise leave a
   fully interactive but invisible menu. */
static K_INT16 getcheatselection(K_INT16 selection, K_INT16 n,
	K_INT16 *chapter, K_INT16 *level)
{
    K_INT16 frame = 0;
    ...
    newkeystatus[SDLK_LEFT] = 0;
    newkeystatus[SDLK_RIGHT] = 0;
    newkeystatus[SDLK_KP2] = 0;
    newkeystatus[SDLK_KP8] = 0;

    for (;;)
    {
	K_INT16 allstate, row, maxchapters, chapterlevels;

	maxchapters = cheatchaptercount();
	if (*chapter >= maxchapters)
	    *chapter = maxchapters-1;
	chapterlevels = cheatlevelsforchapter(*chapter);
	if (*level >= chapterlevels)
	    *level = chapterlevels-1;

	gfxDrawBack();
	drawmenu(304,224,menu);
	strcpy((char *)&textbuf[0],"Cheat menu");
	textprint(140,6+n,112);
	strcpy((char *)&textbuf[0],"Arrows adjust, Esc closes");
	textprint(78,17+n,48);

	row = CHEAT_MENU_FIRST_Y+n;
	allstate = allcheatsenabled();
	if (allstate)
	    strcpy((char *)&textbuf[0],"Toggle all                 ON");
	else if (anycheatsenabled())
	    strcpy((char *)&textbuf[0],"Toggle all              MIXED");
	else
	    strcpy((char *)&textbuf[0],"Toggle all                OFF");
	textprint(67,row,32); row += CHEAT_MENU_ROW_HEIGHT;
	drawcheattoggle("God mode",cheat_godmode,row); row += CHEAT_MENU_ROW_HEIGHT;
	drawcheattoggle("Noclip",cheat_noclip,row); row += CHEAT_MENU_ROW_HEIGHT;
	drawcheattoggle("Money refill",cheat_infinite_money,row); row += CHEAT_MENU_ROW_HEIGHT;
	drawcheattoggle("Key refill",cheat_all_keys,row); row += CHEAT_MENU_ROW_HEIGHT;
	drawcheattoggle("Max weapons",cheat_max_weapons,row); row += CHEAT_MENU_ROW_HEIGHT;
	drawcheattoggle("Powerup refill",cheat_infinite_powerups,row); row += CHEAT_MENU_ROW_HEIGHT;
	drawcheattoggle("Life refill",cheat_extra_lives,row); row += CHEAT_MENU_ROW_HEIGHT;
	drawcheattoggle("Compass",cheat_compass,row); row += CHEAT_MENU_ROW_HEIGHT;
	strcpy((char *)&textbuf[0],"Restore full health");
	textprint(67,row,32); row += CHEAT_MENU_ROW_HEIGHT;
	strcpy((char *)&textbuf[0],"Next board");
	textprint(67,row,32); row += CHEAT_MENU_ROW_HEIGHT;
	sprintf((char *)&textbuf[0],"Chapter %d: %s",*chapter+1,
		cheatchaptername(*chapter));
	textprint(67,row,32); row += CHEAT_MENU_ROW_HEIGHT;
	sprintf((char *)&textbuf[0],"Level %d of %d",*level+1,chapterlevels);
	textprint(67,row,32); row += CHEAT_MENU_ROW_HEIGHT;
	sprintf((char *)&textbuf[0],"Warp to Chapter %d, Level %d",
		*chapter+1,*level+1);
	textprint(67,row,112); row += CHEAT_MENU_ROW_HEIGHT;
	strcpy((char *)&textbuf[0],"Return to game");
	textprint(67,row,32);
	drawcheatselector(selection,n,frame);
	finalisemenu();
	gfxSwapBuffers();

	PollInputs();
	if (newkeystatus[SDLK_ESCAPE])
	{
	    newkeystatus[SDLK_ESCAPE] = 0;
	    ksay(27);
	    return (-selection)-1;
	}
	if (newkeystatus[SDLK_UP] || newkeystatus[SDLK_KP8])
	{
	    selection--;
	    if (selection < 0)
		selection = CHEAT_MENU_ITEMS-1;
	    newkeystatus[SDLK_UP] = 0;
	    newkeystatus[SDLK_KP8] = 0;
	    ksay(27);
	}
	if (newkeystatus[SDLK_DOWN] || newkeystatus[SDLK_KP2])
	{
	    selection++;
	    if (selection >= CHEAT_MENU_ITEMS)
		selection = 0;
	    newkeystatus[SDLK_DOWN] = 0;
	    newkeystatus[SDLK_KP2] = 0;
	    ksay(27);
	}
	if (newkeystatus[SDLK_LEFT] || newkeystatus[SDLK_RIGHT])
	{
	    K_INT16 direction = newkeystatus[SDLK_RIGHT] ? 1 : -1;
	    if (selection == 11)
	    {
		*chapter += direction;
		if (*chapter < 0)
	...
	if (newkeystatus[SDLK_RETURN] || newkeystatus[SDLK_KP_ENTER] ||
	    newkeystatus[SDLK_SPACE])
	{
	    newkeystatus[SDLK_RETURN] = 0;
	    newkeystatus[SDLK_KP_ENTER] = 0;
	    newkeystatus[SDLK_SPACE] = 0;
...
/* Ctrl+Alt+F1 cheat menu.  Resource toggles refill rather than erase the
   player's inventory when disabled, so turning a cheat off cannot destroy a
   legitimately acquired item. */

void ingamecheatmenu()
{
    static K_INT16 selection = 0;
    K_INT16 choice, done, n, chapter, level, chapterlevels;
    K_INT32 savedclock;
    ...
    while (!done)
    {
	choice = getcheatselection(selection,n,&chapter,&level);
	if (choice < 0)
	{
	    selection = (-choice)-1;
	    done = 1;
	...
	else
	{
		...
		case 0:
		    ...
		    break;
		case 1: cheat_godmode ^= 1; if (cheat_godmode) cheated++; break;
		case 2: cheat_noclip ^= 1; if (cheat_noclip) cheated++; break;
		case 3: cheat_infinite_money ^= 1; if (cheat_infinite_money) cheated++; break;
		case 4: cheat_all_keys ^= 1; if (cheat_all_keys) cheated++; break;
		case 5: cheat_max_weapons ^= 1; if (cheat_max_weapons) cheated++; break;
		case 6: cheat_infinite_powerups ^= 1; if (cheat_infinite_powerups) cheated++; break;
		case 7: cheat_extra_lives ^= 1; if (cheat_extra_lives) cheated++; break;
		case 8:
		    cheat_compass ^= 1;
		    if (cheat_compass)
			cheated++;
		...
		case 12:
		    level++;
		    if (level >= cheatlevelsforchapter(chapter))
			level = 0;
		    break;
		case 13:
		    cheat_warp_board = chapter*10+level;
		    cheated++;
		    done = 1;
		    break;
		case 14:
    ...
    wipeoverlay(0,0,361,statusbaryoffset);
    linecompare(statusbar);
    newkeystatus[SDLK_F1] = 0;
    newkeystatus[newkeydefs[CONTROL_CHEAT_MENU]] = 0;
    cheat_menu_requested = 0;
}
...
K_INT16 mainmenu()
{
    ...
    {
	if ((mainmenuplace = getselection(88,47,mainmenuplace,10)) >= 0)
	{
	    if (mainmenuplace == 0)
		if ((k = newgamemenu()) >= 0)
		{
	    ...
	    if (mainmenuplace == 4) controlsmenu();
	    if (mainmenuplace == 5) helpmenu();
	    if (mainmenuplace == 6) bigstorymenu();
	    if (mainmenuplace == 7) orderinfomenu();
	    if (mainmenuplace == 8) creditsmenu();
	    if (mainmenuplace == 9) done = areyousure();
	    if (done == 0)
	    {
...
static void togglefullscreen(void)
{
    ...
    if ((surface == NULL) || !SDL_WM_ToggleFullScreen(surface))
	fprintf(stderr,"Unable to toggle fullscreen mode: %s\n",SDL_GetError());
    else
...
void PollInputs() {
    SDL_Event event;
    int sk;
    static K_UINT16 cheatmodifiers = 0;
    static K_INT16 cheatf1down = 0;
	...
	{	      
		...
		if (sk == SDLK_F1)
		{
		    if (!cheatf1down && ingame &&
			((((event.key.keysym.mod&KMOD_CTRL) != 0) &&
			  ((event.key.keysym.mod&KMOD_ALT) != 0)) ||
			 ((cheatmodifiers&3) && (cheatmodifiers&12))))
